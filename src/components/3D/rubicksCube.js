...
export class RubicksCube {
  ...
  #snapAxis(axis) {
    const abs = axis
      .clone()
      .set(Math.abs(axis.x), Math.abs(axis.y), Math.abs(axis.z));
  ...
  randomizeMove() {
    ...
    const randomCube =
      this.cubes.children[
        Math.floor(Math.random() * this.cubes.children.length)
      ];
  ...
  rotateFace(camera, faceGroup, rotationAxis, hitNormal, delta) {
    ...
    const sign = Math.sign(
      dragDir.dot(new THREE.Vector3().crossVectors(rotationAxis, hitNormal)),
    );
